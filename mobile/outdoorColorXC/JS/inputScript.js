...
function getOverWdith(){
    ...
    $("#partsG2").text(price);
    //求得型材的合计金额
    $("#partsG3").text(accMul($("#partsG2").text() , Math.ceil(((screenWidth*2)+(screenHeight*2))/600)));
}
...
//2.2 第2行数据 -[磁铁]
function getLine2(){
    var num = accMul(moduleWidthNumber,moduleHeightNumber); //板总数量
...
//2.4 第4行数据 -[发送卡]
function getLine4(){
    num = Math.ceil((accMul(WidthSumPixel,HeightSumPixel))/1300000); //发送卡数量
    $("#partsD1").text(num);
...
function getLine5(){
    ...
    receptionCard =  accMul(Math.ceil(moduleWidthNumber/widthNumber), Math.ceil(moduleHeightNumber/heightNumber));
...
//2.6 第6行数据 -[DIV线]
function getLine6(){
    var num = Math.ceil((accMul(WidthSumPixel,HeightSumPixel))/1300000); //发送卡数量
...
function getLine9(){
    ...
    //接头数量/4 向上取整
    var number = (accMul(Math.floor(screenWidth/300),2))+(accMul(Math.floor(screenHeight/300),2));
...
function getLine10(){
    ...
    //如果是五排高以外用方管
    if(screenHeight>600){
        number = accMul(Math.ceil(screenHeight/600),(Math.ceil((moduleWidthNumber+1+Math.floor(screenWidth/300))/(Math.floor(600/(screenHeight+3))))));
    }else{
