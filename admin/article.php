...
if($_POST['keyword'] OR $_POST['cid']){
	header('Location:article.php?cid='.$_POST["cid"].'&keyword='.$_POST['keyword'].'&searchtype='.$_POST['searchtype']);
}
$where = null;
if($_GET['keyword']){
...
$articlecount = $articledata->GetArticleList($cid,"aid desc",true,$where);
$total = count($articlecount);
$take = 20;
$skip = ($curpage - 1) * $take;
$totalpage = (int)($total % $take == 0 ? $total / $take : $total / $take + 1);
$articlelist = $articledata->TakeArticleList($cid,$skip,$take,"aid desc",true,$where);
$action = $_POST["action"];
if($action == "delete")
{
    $idarr = $_POST["chk"];
	if(count($idarr) > 0)
	{
		foreach($idarr as $id)
		{
			if(is_numeric($id))
			{
				$article_info = $articledata->GetArticle($id,true);
				$sql = "DELETE FROM yiqi_article WHERE aid = '$id' limit 1";
				$yiqi_db->query(CheckSql($sql));
				$attachdata->removeAttachByItemId($id, "article");
				//remove keywords
				$keywordsdata = new Keywords;
				$keywordsdata->RemoveKeyword($article_info->seokeywords);
				echo "
