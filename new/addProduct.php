...
$sql = "select pid,name,content,adddate,seokeywords,seodescription,thumb from yiqi_product";
$zhixing = @mysqli_query($conn, $sql);
while ($rows = @mysqli_fetch_row($zhixing)) {
    //调用创建文件方法
    newfile($rows[0], $rows[1], $rows[2], $rows[3], $rows[4], $rows[5], $rows[6]);
}

//3.关闭数据库资源
mysqli_close($conn);

function newfile($fileNames, $name, $content, $adddate, $seokeywords, $seodescription,$thumb)
{
    //判断file文件夹下以这个命名的文件是否存在
    $newfiles = './product/' . $fileNames;
