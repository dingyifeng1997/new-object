...
class Product
{
	...
	function GetProductList($cid,$orderby="adddate desc",$all=false,$where=null)
	{
	    global $yiqi_db;
		...
		if($exist == 1)
		{
			...
			if($all)
			{
				return $yiqi_db->get_results(CheckSql("select * from yiqi_product where cid in ($cids) $andwhere order by $orderby "));
			}
			else
			{
				return $yiqi_db->get_results(CheckSql("select * from yiqi_product where cid in ($cids) and adddate <= '".date("Y-m-d H:i:s")."' $andwhere order by $orderby "));
			}
		}
		else
		{
			if($all)
			{
				return $yiqi_db->get_results(CheckSql("select * from yiqi_product $wherewhere order by $orderby"));
			}
			else
			{
				return $yiqi_db->get_results(CheckSql("select * from yiqi_product where adddate <= '".date("Y-m-d H:i:s")."' $andwhere order by $orderby"));
			}
		}
	}
	
	function TakeProductList($cid=0,$skip=0,$take=10,$orderby="adddate desc",$all=false,$where=null)
	{
	    global $yiqi_db;
	    $categorydata = new Category();
		//where
		...
		if($exist == 1)
		{
			...
			if($all)
			{
				return $yiqi_db->get_results(CheckSql("select * from yiqi_product where cid in ($cids) $andwhere order by $orderby limit $skip,$take "));
			}
			else
			{
				return $yiqi_db->get_results(CheckSql("select * from yiqi_product where cid in ($cids) $andwhere and adddate <= '".date("Y-m-d H:i:s")."' order by $orderby limit $skip,$take "));
			}
		}
		else
		{
			if($all)
			{
				return $yiqi_db->get_results(CheckSql("select * from yiqi_product $wherewhere order by $orderby limit $skip,$take"));
			}
			else
			{
				return $yiqi_db->get_results(CheckSql("select * from yiqi_product where adddate <= '".date("Y-m-d H:i:s")."' $andwhere order by $orderby limit $skip,$take"));
			}
		}
	}
