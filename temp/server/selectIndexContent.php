...
function select(){
    ...
    $sql = "select aid,title,cid,content,adddate,thumb,seodescription from yiqi_article where cid=2 order by aid desc limit $pageN,$limt";
    $zhixing = mysqli_query($conn,$sql);
    $str = "";
    while($rows = @mysqli_fetch_row($zhixing)){
//        { "name":"菜鸟教程" , "url":"www.runoob.com" }
        $str.= "{'"."aid':'"."{$rows[0]}"."','"."title':'"."{$rows[1]}"."','"."cid':'"."{$rows[2]}"."','"."content':'"."{$rows[3]}"."','"."adddate':'"."{$rows[4]}"."','"."thumb':'"."{$rows[5]}"."','"."seodescription':'"."{$rows[6]}"."'"."},";
    }
